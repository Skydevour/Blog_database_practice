                    ...
                    <c:forEach items="${searchList}" var="search">
                        <div class="ui padded vertical segment m-padded-tb-large">
                            <div class="ui mobile reversed stackable grid">
                                ...
                                <div class="eleven wide column">
                                    <h3 class="ui header" >
                                        <p onclick="showSingleBlog(${search.vId})" class="m-black">
                                            <c:if test="${fn:length(search.content) >0}">
                                                <a href="#" class="item">
                                                        ${fn:substring(search.content, 0, 10)}...
                                                </a>
                                            </c:if>
                                        </p>
                                    </h3>
                                    <p class="m-text">
                                        <c:if test="${fn:length(search.content)>0 && fn:length(search.content)<=50}">
                                            ${fn:substring(search.content, 0, 50)}
                                        </c:if>
                                        <c:if test="${fn:length(search.content) >50}">
                                            ${fn:substring(search.content, 0, 50)}...
                                        </c:if>
                                    </p>
                                    <div class="ui grid">
                                        <div class="eleven wide column">
                                            <div class="ui mini horizontal link list">
                                                <div class="item">
                                                    <div class="content"><a href="#" class="header">${search.userName}</a></div>
                                                </div>
                                                <div class="item">
                                                    <i class="calendar icon"></i><fmt:formatDate value="${search.datetime}" pattern="yyyy-MM-dd HH:mm:ss"/>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
